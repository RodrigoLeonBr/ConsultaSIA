...
<div class="flex items-center justify-between">
    <div>
        ...
        <p class="text-gray-600 mt-1">Comparação S_PA.DBF + S_RUB.DBF × MySQL — competência {{ substr($result['competence'], 0, 4) }}-{{ substr($result['competence'], 4, 2) }} · {{ $result['total_rub'] ?? 0 }} rubricas</p>
    </div>
    <div class="flex space-x-3">
        ...
        <a href="{{ route('procedimento.index') }}" class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700">
            Ver procedimentos
        </a>
    </div>
</div>
    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
        ...
        <div class="grid grid-cols-2 md:grid-cols-6 gap-4">
            <div class="bg-white rounded-lg shadow p-4 text-center">
                ...
                <div class="text-xs text-gray-500 uppercase mt-1">Linhas DBF</div>
            </div>
            <div class="bg-white rounded-lg shadow p-4 text-center">
                <div class="text-2xl font-bold text-gray-700">{{ $result['total_unique'] }}</div>
                <div class="text-xs text-gray-500 uppercase mt-1">Códigos únicos</div>
            </div>
            <div class="bg-white rounded-lg shadow p-4 text-center">
        ...
        @php $sRub = $result['s_rub'] ?? ['created' => [], 'updated' => [], 'unchanged' => 0, 'only_mysql' => []]; @endphp

        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="px-6 py-4 border-b border-gray-200">
                <h2 class="text-lg font-semibold text-indigo-700">Tabela s_rub (sincronizada automaticamente)</h2>
                <p class="text-sm text-gray-500 mt-1">
                    {{ count($sRub['created']) }} criada(s) · {{ count($sRub['updated']) }} atualizada(s) · {{ $sRub['unchanged'] }} inalterada(s) · {{ count($sRub['only_mysql']) }} só no MySQL
                </p>
            </div>

            @if (count($sRub['created']) > 0)
                <div class="px-6 py-4 border-b">
                    <h3 class="text-sm font-semibold text-green-700 mb-2">Novas rubricas ({{ count($sRub['created']) }})</h3>
                    <div class="overflow-x-auto max-h-40">
                        <table class="min-w-full text-sm divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">RUB_ID</th>
                                    <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">RUB_DC</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-100">
                                @foreach ($sRub['created'] as $item)
                                    <tr>
                                        <td class="px-4 py-2 font-mono">{{ $item['rub_id'] }}</td>
                                        <td class="px-4 py-2">{{ $item['rub_dc'] }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
            ...
            @if (count($sRub['updated']) > 0)
                <div class="px-6 py-4 border-b">
                    <h3 class="text-sm font-semibold text-amber-700 mb-2">Rubricas atualizadas ({{ count($sRub['updated']) }})</h3>
                    <div class="space-y-4 max-h-64 overflow-y-auto">
                        @foreach ($sRub['updated'] as $item)
                            <div>
                                <p class="text-sm font-mono font-medium">{{ $item['rub_id'] }} <span class="font-sans text-gray-600">{{ $item['rub_dc'] }}</span></p>
                                <table class="mt-1 min-w-full text-sm border border-gray-200 rounded">
                                    <thead class="bg-gray-50">
                                        <tr>
                                            <th class="px-3 py-1 text-left text-xs text-gray-500">Campo</th>
                                            <th class="px-3 py-1 text-left text-xs text-gray-500">MySQL (antes)</th>
                                            <th class="px-3 py-1 text-left text-xs text-gray-500">DBF (agora)</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @foreach ($item['diffs'] as $diff)
                                            <tr>
                                                <td class="px-3 py-1">{{ $diff['label'] }}</td>
                                                <td class="px-3 py-1 text-red-700 bg-red-50">{{ $diff['mysql'] ?: '—' }}</td>
                                                <td class="px-3 py-1 text-green-700 bg-green-50">{{ $diff['dbf'] ?: '—' }}</td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                </table>
            ...
            @if (count($sRub['only_mysql']) > 0)
                <div class="px-6 py-4">
                    <h3 class="text-sm font-semibold text-blue-700 mb-2">Rubricas só no MySQL ({{ count($sRub['only_mysql']) }})</h3>
                    <div class="overflow-x-auto max-h-32">
                        <table class="min-w-full text-sm divide-y divide-gray-200">
                            <tbody class="divide-y divide-gray-100">
                                @foreach ($sRub['only_mysql'] as $item)
                                    <tr>
                                        <td class="px-4 py-2 font-mono">{{ $item['rub_id'] }}</td>
                                        <td class="px-4 py-2">{{ $item['rub_dc'] }}</td>
                                    </tr>
        ...
        @if (count($result['created']) > 0)
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="px-6 py-4 border-b border-gray-200">
                    ...
                    <p class="text-sm text-amber-700 mt-1">Complete <strong>Financiamento</strong> manualmente se necessário.</p>
                </div>
                        ...
                        <tbody class="divide-y divide-gray-200">
                            @foreach ($result['created'] as $item)
                                <tr>
                                    ...
                                    <td class="px-6 py-3 text-sm">{{ $item['procedimento'] }}</td>
        ...
        @if (count($result['changed']) > 0)
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <form method="POST" action="{{ route('procedimento.import.apply') }}">
                    @csrf
                            ...
                            <div class="p-6">
                                <div class="flex items-start space-x-3 mb-3">
                                    ...
                                    <div>
                                        ...
                                        <span class="text-sm text-gray-600 ml-2">{{ $item['procedimento'] }}</span>
                                    </div>
                                </div>
                                        ...
                                        <tbody class="divide-y divide-gray-100">
                                            @foreach ($item['diffs'] as $diff)
                                                <tr>
                                                    ...
                                                    <td class="px-4 py-2 text-red-700 bg-red-50">{{ $diff['mysql'] ?: '—' }}</td>
                                                    <td class="px-4 py-2 text-green-700 bg-green-50">{{ $diff['dbf'] ?: '—' }}</td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                    </table>
        ...
        @if (count($result['only_mysql']) > 0)
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="px-6 py-4 border-b border-gray-200">
                    ...
                    <p class="text-sm text-gray-500 mt-1">Não vieram no DBF — não foram alterados nem removidos.</p>
                </div>
                <div class="overflow-x-auto max-h-64">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50 sticky top-0">
