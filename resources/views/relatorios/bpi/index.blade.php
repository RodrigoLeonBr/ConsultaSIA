...
@section('title', 'Produção Individualizada')

@section('header')
<div class="flex items-center justify-between">
    <div>
        <h1 class="text-3xl font-bold text-gray-900">Produção Individualizada</h1>
        <p class="text-gray-600 mt-1">Relatórios por prestador/profissional (tabela s_bpi)</p>
    </div>
</div>
@endsection
...
@section('content')
            ...
            <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
                <div class="flex">
                    ...
                    <div class="ml-3 flex-1">
                        <h3 class="text-sm font-medium text-blue-800">Produção Individualizada por Prestador</h3>
                        <div class="mt-2 text-sm text-blue-700">
                            <p>Este sistema gera relatórios da <strong>produção individualizada</strong> (tabela s_bpi) com 2.115.709 registros.</p>
                            <p class="mt-1">Inclui dados de profissional (CNS), paciente e atendimento. Para produção hospitalar agregada, acesse:
                                <a href="{{ route('relatorios.index') }}" class="font-medium text-blue-800 hover:text-blue-900 underline">
                                    Relatórios Produção →
                                </a>
                            </p>
                        </div>
    ...
    <script>
        ...
        const loadLookupData = (field, search) => RelatoriosBase.loadLookupData(field, search, '{{ route("relatorios.bpi.lookup") }}');
        ...
        async function loadAvailableFields() {
            try {
                const url = '{{ route("relatorios.bpi.fields") }}';
        ...
        function updateFilterValueInput() {
            ...
            if (field && field.options) {
                ...
                container.innerHTML = `
                    <label class="block text-sm font-medium text-gray-700 mb-1">Valor</label>
                    <select id="filter-value" class="w-full border border-gray-300 rounded-md px-3 py-2">
                        <option value="">Selecione...</option>
                        ${optionsHtml}
                    </select>
                `;
            } else if (fieldKey === 'BPI_IDADE') {
                container.innerHTML = `
                    <label class="block text-sm font-medium text-gray-700 mb-1">Valor</label>
        ...
        function saveFilter() {
            ...
            const displayOperator = operatorLabels[operator] || operator;
            ...
            const filter = {
                ...
                operator: operator,
                value: value,
                label: `${fieldConfig.label} ${displayOperator} ${displayValue}`
            };
        ...
        async function generateReport(format) {
            ...
            try {
                ...
                const routeUrl = isMatrixView ? 
                    '{{ route("relatorios.bpi.generate-matrix") }}' : 
                    '{{ route("relatorios.bpi.generate") }}';
