...
@section('title', 'Relatório de Produção e-SUS')

@section('header')
<div class="flex items-center justify-between">
    <div>
        <h1 class="text-3xl font-bold text-gray-900">Relatório de Produção e-SUS</h1>
        <p class="text-gray-600 mt-1">Produção SIGTAP por unidade e procedimento (tabela s_esus) — sem valores financeiros</p>
    </div>
...
@section('content')
    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
            <div class="flex">
                ...
                <div class="ml-3">
                    <h3 class="text-sm font-medium text-blue-800">Relatório de Produção e-SUS (SIGTAP)</h3>
                    <div class="mt-2 text-sm text-blue-700">
                        <p>Agrupa a produção importada do e-SUS (tabela <code>s_esus</code>) por unidade e procedimento SIGTAP.</p>
                        <p class="mt-1">Use o campo <strong>Considerar e-SUS</strong> (= Sim) para excluir unidades cuja produção vem de outra fonte (ex.: SIA).</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-emerald-50 border border-emerald-200 rounded-lg p-4 mb-6">
            <label class="flex items-center">
                <input type="checkbox" id="esus-ativo-only" checked class="mr-2 rounded border-gray-300">
                <span class="text-sm font-medium text-emerald-800">
                    Considerar apenas unidades marcadas como "Considerar e-SUS" (exclui esus_ativo = Não)
                </span>
            </label>
            <p class="text-xs text-emerald-600 mt-1">Desmarque para incluir todas as unidades, inclusive as que têm a produção lida de outra fonte (ex.: SIA).</p>
        </div>
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
            <div class="p-6">
                    ...
                    <div class="lg:col-span-2">
                        <div class="flex justify-between items-center mb-4">
                            ...
                            <button id="add-filter"
                                    style="background-color: #3b82f6 !important; color: white !important; padding: 0.25rem 0.75rem !important; border-radius: 0.25rem !important; font-size: 0.875rem !important; display: inline-block !important; min-width: 140px !important; white-space: nowrap !important; height: 32px !important; line-height: 20px !important; border: none !important; cursor: pointer !important;"
                ...
                <div class="mt-6 flex flex-wrap gap-3">
                    <button id="generate-report" class="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700">🔍 Gerar Relatório e-SUS</button>
            ...
            <div class="p-6">
                ...
                <div id="loading-indicator" class="hidden text-center py-8">
                    <div class="inline-flex items-center">
                        ...
                        <span class="text-gray-600">Processando relatório e-SUS...</span>
                    </div>
                </div>
                <div id="results-container">
                    <p class="text-gray-500 text-center py-8">Configure os campos e filtros acima e clique em "Gerar Relatório e-SUS".</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        const loadLookupData     = (field, search) =>
            RelatoriosBase.loadLookupData(field, search, '{{ route("relatorios.esus.lookup") }}');
        ...
        async function loadAvailableFields() {
            ...
            try {
                ...
                const response = await fetch('{{ route("relatorios.esus.fields") }}');
        ...
        function updateValueContainer(operator) {
            ...
            if (operator === 'between') {
                ...
                if (v1)  v1.placeholder  = 'Valor inicial...';
                v2c?.classList.remove('hidden');
                if (v2)  v2.placeholder  = 'Valor final...';
            } else {
        ...
        function updateFilterOperators() {
            ...
            const labels = { '=':'Igual a','>':'Maior que','<':'Menor que','>=':'Maior ou igual','<=':'Menor ou igual','like':'Contém','starts_with':'Inicia com','ends_with':'Termina em','between':'Faixa (início → fim)','in':'Em lista' };
        ...
        function doSaveFilter() {
            ...
            if (operator === 'between') {
                ...
                if (!v1) { alert('Preencha a lista separada por vírgula.'); return; }
                value = v1.split(',').map(s => s.trim()).filter(Boolean);
                label = fieldLabel + ' em: ' + v1;
            } else {
        ...
        async function generateReport(format) {
            ...
            const payload   = {
                ...
                filters: (() => {
                    const all = appliedFilters.map(f => ({ field: f.field, operator: f.operator, value: f.value }));
                    if (document.getElementById('esus-ativo-only')?.checked) {
                        all.push({ field: 'filter_esus_ativo', operator: '=', value: true });
                    }
                    return all;
                })(),
                format
            };
            const routeUrl  = isMatrix ? '{{ route("relatorios.esus.generate-matrix") }}' : '{{ route("relatorios.esus.generate") }}';
            ...
            try {
                ...
                if (format === 'html') {
                    ...
                    await handleFileDownload(response, format, 'relatorio_esus.' + (format === 'excel' ? 'xlsx' : format));
                }
            } catch (err) {
