    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Editar Prestador: ' . $prestador->re_cnome) }}
            </h2>
            <div class="flex space-x-3">
                <a href="{{ route('prestador.show', $prestador) }}" class="bg-green-600 text-white px-4 py-2 rounded-md hover:bg-green-700 transition-colors duration-200">
                    Ver Detalhes
                </a>
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('prestador.update', $prestador) }}">
                        @csrf
                        @method('PATCH')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="text" 
                                       id="re_cunid" 
                                       name="re_cunid" 
                                       value="{{ old('re_cunid', $prestador->re_cunid) }}"
                                       maxlength="7"
                            ...
                            <div>
                                ...
                                <input type="text" 
                                       id="re_cnome" 
                                       name="re_cnome" 
                                       value="{{ old('re_cnome', $prestador->re_cnome) }}"
                                       maxlength="35"
                            ...
                            <div>
                                ...
                                <select id="re_tipo" 
                                    ...
                                    <option value="P" {{ old('re_tipo', $prestador->re_tipo) == 'P' ? 'selected' : '' }}>Privado/Único</option>
                                    <option value="U" {{ old('re_tipo', $prestador->re_tipo) == 'U' ? 'selected' : '' }}>Unidade Básica</option>
                                    <option value="M" {{ old('re_tipo', $prestador->re_tipo) == 'M' ? 'selected' : '' }}>Hospital Municipal</option>
                                </select>
                                @error('re_tipo')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <select id="tipouni" 
                                    ...
                                    <option value="">Selecione...</option>
                                    <option value="M" {{ old('tipouni', $prestador->tipouni) == 'M' ? 'selected' : '' }}>Municipal</option>
                                    <option value="F" {{ old('tipouni', $prestador->tipouni) == 'F' ? 'selected' : '' }}>Filantrópico</option>
                                    <option value="P" {{ old('tipouni', $prestador->tipouni) == 'P' ? 'selected' : '' }}>Particular</option>
                                    <option value="E" {{ old('tipouni', $prestador->tipouni) == 'E' ? 'selected' : '' }}>Estadual</option>
                                </select>
                                @error('tipouni')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <select id="relatorio" 
                                    ...
                                    <option value="">Selecione...</option>
                                    <option value="Atenção Básica" {{ old('relatorio', $prestador->relatorio) == 'Atenção Básica' ? 'selected' : '' }}>Atenção Básica</option>
                                    <option value="Urgência e Emergência" {{ old('relatorio', $prestador->relatorio) == 'Urgência e Emergência' ? 'selected' : '' }}>Urgência e Emergência</option>
                                    <option value="Atenção Psicossocial" {{ old('relatorio', $prestador->relatorio) == 'Atenção Psicossocial' ? 'selected' : '' }}>Atenção Psicossocial</option>
                                    <option value="Atenção Ambulatorial Especializada" {{ old('relatorio', $prestador->relatorio) == 'Atenção Ambulatorial Especializada' ? 'selected' : '' }}>Atenção Ambulatorial Especializada</option>
                                    <option value="Hospitalar" {{ old('relatorio', $prestador->relatorio) == 'Hospitalar' ? 'selected' : '' }}>Hospitalar</option>
                                </select>
                                @error('relatorio')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div class="flex items-center">
                                <div class="flex items-center h-5">
                                    <input type="checkbox" 
                                           ...
                                           name="ativo" 
                                           value="1"
                                           {{ old('ativo', $prestador->ativo) ? 'checked' : '' }}
                            ...
                            <div class="flex items-center">
                                <div class="flex items-center h-5">
                                    <input type="checkbox"
                                           ...
                                           name="esus_ativo"
                                           value="1"
                                           {{ old('esus_ativo', $prestador->esus_ativo) ? 'checked' : '' }}
                                           class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded">
                                </div>
                                <div class="ml-3 text-sm">
