...
import { getRubricas } from '../api';
import { useServerTable } from '../hooks/useServerTable';

interface Rubrica { id: string; descricao: string; }

const COLUMNS: ColumnDef<Rubrica, unknown>[] = [
  { accessorKey: 'id', header: 'ID', cell: info => <span className="font-mono text-sm">{info.getValue<string>()}</span> },
...
export function RubricasPage() {
  ...
  const [data, setData] = useState<Rubrica[]>([]);
  ...
  const load = useCallback(async (s: string, p: number, ps: number) => {
    ...
    try {
      const res = await getRubricas(s, p, ps);
      setData(res.data ?? []);
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-3 items-end">
        <Input wrapperClassName="flex-1 max-w-sm" placeholder="Buscar por ID ou descrição..." value={search} onChange={e => setSearch(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleApply()} />
